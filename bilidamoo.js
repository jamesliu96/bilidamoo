...
io.on('connection', function(client) {
    console.log("[系统] 已连接感谢鸡。".bold.green);
    client.on('disconnect', function(){
        console.log("[系统] 与感谢鸡连接断开。".bold.green);
    });
});
io.listen(3000);
...
socket.on('comment', function(data) {
    ...
    switch (data.cmd) {
        ...
        case 'SEND_GIFT':
            ...
            if ((info.giftName == '233' || info.giftName == '666' || info.giftName == 'FFF') && parseInt(info.num) < 5) { break; }
            
            // say('感谢' + info.uname + info.action + '的' + info.num + '个' + info.giftName);

            thank('感谢' + info.uname + info.action + '的' + info.num + '个' + info.giftName + '喵~');
